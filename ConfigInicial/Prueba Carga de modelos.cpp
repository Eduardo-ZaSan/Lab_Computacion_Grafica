...
void Animation() {
	...
	if (AnimBall) {
		...
		if (AnimBall) {
			rotBall += 0.001f * direccion; // Incrementa o decrementa rotBall seg n la direcci n

			// Cambiar la direcci n si se alcanza el l mite
			if (rotBall >= maxRot) {
			...
			rotBall = 0.0f; // Reinicia el valor si la animaci n no est  activa
			direccion = 1;  // Reinicia la direcci n a subiendo
		}
